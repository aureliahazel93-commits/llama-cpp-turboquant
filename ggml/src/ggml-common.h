...
// AngelSlim Tequila: 2.0 bpw deadzone-aware ternary with imatrix weighting
typedef struct {
    uint8_t qs[QK_K/4];                   // 2 bits per element, 256 weights
    ggml_half d;                           // fp16 scale (absmax per block)
...
// PlanarQuant 4-bit: 2D Givens rotation + 3-bit quantized + 1-bit signs
#define QK_PLANAR4 128
#define NL_PLANAR4 (QK_PLANAR4 / 16)
#define NL_PLANAR4_VEC (QK_PLANAR4 / 4)
typedef struct {
    ggml_half    norm;
    ggml_half    rnorm;
    uint8_t    qs[QK_PLANAR4 / 2];
    uint8_t    signs[QK_PLANAR4 / 8];
} block_planar4_0;
static_assert(sizeof(block_planar4_0) == 2*sizeof(ggml_half) + QK_PLANAR4/2 + QK_PLANAR4/8, "wrong planar4_0 block size/padding");

// IsoQuant 4-bit: quaternion 4D rotation + 3-bit quantized + 1-bit signs
#define QK_ISO4 128
#define NL_ISO4 (QK_ISO4 / 16)
#define NL_ISO4_VEC (QK_ISO4 / 4)
typedef struct {
    ggml_half    norm;
    ggml_half    rnorm;
    uint8_t    qs[QK_ISO4 / 2];
    uint8_t    signs[QK_ISO4 / 8];
} block_iso4_0;
static_assert(sizeof(block_iso4_0) == 2*sizeof(ggml_half) + QK_ISO4/2 + QK_ISO4/8, "wrong iso4_0 block size/padding");

// STQ1_0: 1.31 bpw structured ternary, 32-entry codebook
#define QK_STQ1_0 256
#define QC_STQ1_0 32
#define GS_STQ1_0 4
typedef struct {
    ggml_half d;
    uint8_t qs[QK_STQ1_0 / 8];
    uint8_t signs[QK_STQ1_0 / 32];
} block_stq1_0;
static_assert(sizeof(block_stq1_0) == sizeof(ggml_half) + QK_STQ1_0/8 + QK_STQ1_0/32, "wrong stq1_0 block size/padding");
...
// STQ1_0 codebook and lookup tables
